...
#ifndef GALOIS_GRAPHS_LC_CSR_Graph_PM_H
#define GALOIS_GRAPHS_LC_CSR_Graph_PM_H
...
public:
  ...
  struct with_id {
    typedef LC_CSR_Graph_PM type;
  };

  template <typename _node_data>
  struct with_node_data {
    typedef LC_CSR_Graph_PM<_node_data, EdgeTy, HasNoLockable, UseNumaAlloc,
  ...
  struct with_edge_data {
    typedef LC_CSR_Graph_PM<NodeTy, _edge_data, HasNoLockable, UseNumaAlloc,
                         HasOutOfLineLockable, FileEdgeTy>
        type;
  };
  ...
  struct with_file_edge_data {
    typedef LC_CSR_Graph_PM<NodeTy, EdgeTy, HasNoLockable, UseNumaAlloc,
  ...
  struct with_no_lockable {
    typedef LC_CSR_Graph_PM<NodeTy, EdgeTy, _has_no_lockable, UseNumaAlloc,
                         HasOutOfLineLockable, FileEdgeTy>
        type;
  };
  template <bool _has_no_lockable>
  using _with_no_lockable =
      LC_CSR_Graph_PM<NodeTy, EdgeTy, _has_no_lockable, UseNumaAlloc,
                   HasOutOfLineLockable, FileEdgeTy>;
  ...
  struct with_numa_alloc {
    typedef LC_CSR_Graph_PM<NodeTy, EdgeTy, HasNoLockable, _use_numa_alloc,
                         HasOutOfLineLockable, FileEdgeTy>
        type;
  };
  template <bool _use_numa_alloc>
  using _with_numa_alloc =
      LC_CSR_Graph_PM<NodeTy, EdgeTy, HasNoLockable, _use_numa_alloc,
                   HasOutOfLineLockable, FileEdgeTy>;
...
public:
  LC_CSR_Graph_PM(LC_CSR_Graph_PM&& rhs) = default;

  LC_CSR_Graph_PM() = default;

  LC_CSR_Graph_PM& operator=(LC_CSR_Graph_PM&&) = default;
  ...
  LC_CSR_Graph_PM(uint32_t _numNodes, uint64_t _numEdges, EdgeNumFnTy edgeNum,
    ...
    if (UseNumaAlloc) {
      ...
      //Added by Chinmay
      currCache.allocateBlocked(cacheSize);
      nextCache.allocateBlocked(cacheSize);
      // edgeIndDataCache.allocateBlocked(numNodes);
      //! [numaallocex]
      ...
      //Added by Chinmay
      currCache.allocateInterleaved(cacheSize);
      nextCache.allocateInterleaved(cacheSize);
      // edgeIndDataCache.allocateInterleaved(numNodes);
  ...
  friend void swap(LC_CSR_Graph_PM& lhs, LC_CSR_Graph_PM& rhs) {
...
//Edit made by Chinmay - removed const
...
*/
  void allocateFrom(const FileGraph& graph) {
    ...
    printf("Graph memory allocation finished\n");
  }
  ...
  void deallocate() {
    ...
    //Added by Chinmay
    currCache.deallocate();
    currCache.destroy();

    nextCache.deallocate();
    nextCache.destroy();
  }
        ...
        graph
            .divideByNode(
                NodeData::size_of::value + EdgeIndData::size_of::value +
                    LC_CSR_Graph_PM::size_of_out_of_line::value,
...
//Added by Chinmay
  void writeToCurrCache(edge_iterator index, GraphNode N){
    currCache[*index] = N;
  }

  void writeToNextCache(edge_iterator index, GraphNode N){
    nextCache[*index] = N;
  }

  GraphNode readFromCurrCache(edge_iterator index){
    return currCache[*index];
  }

  void swapCache(){
    // std::swap(currCache, nextCache);
